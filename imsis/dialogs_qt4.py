...
This module contains methods that are used for graphical user interaction. (DEPRECIATED)
"""
...
class Window(QtGui.QWidget):
    ...
    def dialog_propertygrid(self, properties, text, info=""):
        ...
        for item in data:
            table.setItem(i, 0, QtGui.QTableWidgetItem(item[0]))
            ...
            if (item[1] == 'True' or item[1] == 'False'):
                ...
                # checkbox.isChecked()
                checkbox.clicked.connect(onStateChanged)

                '''
                combo_box_options = ['True', 'False']
                ...
                table.setItem(i, 1, QtGui.QTableWidgetItem(item[1]))
            i = i + 1
...
class MultiButtonWidget(QtGui.QWidget):
...
class CheckListWidget(QtGui.QDialog):
...
class RadioButtonListWidget(QtGui.QDialog):
    ...
    def __init__(
        ...
        for string, datatype in datalist:
            self.button_name = QtGui.QRadioButton("{}".format(string))
...
class ComboBoxWidget(QtGui.QDialog):
    ...
    def __init__(
        ...
        # self.listView = QtGui.QListView()
        self.button_group = QtGui.QComboBox()
...
class ImageListViewWidget(QtGui.QListWidget):
...
class ImageListViewForm(QtGui.QMainWindow):
...
class Dialogs(object):
    ...
    def openfile_dialog(path='/', windowtext='Open File Dialog', alwaysontop=True):
        ...
        filename = QtGui.QFileDialog.getOpenFileName(window, windowtext, path)
        return filename
    ...
    def openfolder_dialog(path='/', windowtext='Open File Dialog', alwaysontop=True):
        ...
        folder = QtGui.QFileDialog.getExistingDirectory(window, windowtext, path)
        return folder
    ...
    def select_singlepoint(img, windowtext="Select Point by double click"):
        ...
        while True:
            ...
            #NOTE cv.getWindowProperty works for opencv-python 4.5.1.48 but not for 4.5.3.56
    ...
    def adjust_mask(img, windowtext="Select Mask"):
        ...
        cv.createTrackbar("Max", windowtext, 255, 255, nothing)
    ...
    def adjust_HSL(img, windowtext="HSL Channels"):
        ...
        cv.createTrackbar("Hue", windowtext, 128, 180, nothing)
        cv.createTrackbar("Sat", windowtext, 128, 255, nothing)
        cv.createTrackbar("Light", windowtext, 0, 255, nothing)
        ...
        while (1):
            ...
            hsl = ims.Image.Adjust.adjust_HSL(frame, Hue, Sat, Light)
        ...
        cv.destroyAllWindows()
        print("Adjust HSL: image, Hue={},Sat={},Light={}".format(Hue, Sat, Light))
        hls = ims.Image.Adjust.adjust_HSL(img, Hue, Sat, Light)
    ...
    def adjust_HSV(img, windowtext="HSV Channels"):
        ...
        while (1):
            ...
            hsv = ims.Image.Adjust.adjust_HSV(frame, Hue, Sat, Val)
        ...
        cv.destroyAllWindows()
        print("Adjust HSV: image, Hue={},Sat={},Value={}".format(Hue, Sat, Val))
        hls = ims.Image.Adjust.adjust_HSV(img, Hue, Sat, Val)
